...
from appv1.crud.users import get_user_by_email, get_user_by_id, create_user_sql, update_password
from appv1.crud.permissions import get_all_permissions
from appv1.schemas.user import ChangePassword, ResponseLoggin, UserLoggin, UserCreate
...
def send_email(to_email: str, subject: str, body: str):
    ...
    headers = {
        ...
        "Authorization": "Bearer mlsn.5561e9965c07e13a4daafbae2350bd7647830d18335f06402e6acef1a6acf0e3"
    }
    data = {
        "from": {
            "email": "MS_ZNVFHE@trial-zr6ke4no0pelon12.mlsender.net"
        },
        "to": [
            {
                "email": to_email
...
async def change_password(data: ChangePassword, db: Session = Depends(get_db)):
    ...
    if code_info['code'] != data.code or time.time() > code_info['expires_at']:
        ...
        raise HTTPException(status_code=400, detail="Código Expiró")
